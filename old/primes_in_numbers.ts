export class G964 {
    ...
    private static formatFactors = (factors: number[]): string => {
        let factorCount: number = 1;
        let lastFactor: number = 0;
        return factors.reduce((acc: string, factor: number, idx: number): string => {
            if (factor !== lastFactor) {
                if (lastFactor !== 0) {
                    acc += factorCount > 1 ? `(${lastFactor}**${factorCount})` : `(${lastFactor})`
                }
...
function testing(n: number, expected: string) {
    assert.equal(convertStringToAny(G964.primeFactors(n)), convertStringToAny(expected));
}
