...
const bitLength = (bits: string): number => {
  const onesLength = bits
    .split('0')
    .filter(x => x.length > 0)
    .sort((a, b) => a.length - b.length)[0].length;
  if (bits.includes('0')) {
    const zerosLength = bits
      .split('1')
      .filter(x => x.length > 0)
      .sort((a, b) => a.length - b.length)[0].length;
...
export const decodeBits = (bits: string): string => {
  ...
  const timeLength: number = bitLength(cleanedBits);
  ...
  Object.entries(transcodeKeys).map((x: string[]): void => {
    cleanedBits = replaceBits(cleanedBits, x);
  });
  return cleanedBits;
};
...
export const decodeMorse = (morseCode: string): string => {
  return morseCode
    .trim()
    .split('   ')
    .map((word: string): string => decodeWord(word))
    .join(' ');
};
